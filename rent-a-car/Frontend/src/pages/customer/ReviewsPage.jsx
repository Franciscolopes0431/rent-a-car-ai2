...
function ReviewsPage() {
  const { bookings, isLoading: bookingsLoading } = useBookings({ history: 'true' }, 100);
  ...
  const load = async () => {
    try { setReviews((await featureService.listReviews()).data); setError(''); }
    catch (requestError) { setError(requestError.response?.data?.message || 'Não foi possível carregar as avaliações.'); }
    finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);

  const eligible = useMemo(() => bookings.filter((booking) =>
    booking.estado === 'concluida' && !reviews.some((review) => review.reservationId === booking.id)
  ), [bookings, reviews]);

  const save = async (event) => {
    event.preventDefault();
    try {
      await featureService.createReview({ reservationId: selected.id, ...form });
      await load();
      setSelected(null); setForm({ rating: 5, comment: '' });
    } catch (requestError) { setError(requestError.response?.data?.message || 'Não foi possível publicar a avaliação.'); }
  };

  const busy = loading || bookingsLoading;
  return <Container className="py-4 rc-customer-page">
    <div className="rc-customer-page-header"><div><span className="rc-eyebrow">Experiência</span><h1>As Minhas Avaliações</h1><p>Partilhe a sua experiência após uma reserva concluída.</p></div>{eligible.length ? <Button variant="warning" onClick={() => setSelected(eligible[0])}>Avaliar uma reserva</Button> : null}</div>
    {error ? <Alert variant="danger">{error}</Alert> : null}
    {busy ? <div className="text-center py-5"><Spinner variant="warning" /></div> : reviews.length === 0 ? <EmptyState title="Sem avaliações" description={eligible.length ? 'Já pode avaliar uma das suas reservas concluídas.' : 'Quando concluir uma reserva, poderá avaliá-la aqui.'} action={eligible.length ? { label: 'Avaliar agora', onClick: () => setSelected(eligible[0]) } : undefined} /> : <Row className="g-4">{reviews.map((review) => <Col md={6} key={review.id}><article className="rc-card h-100"><div className="d-flex justify-content-between"><h2 className="h5">{review.reservation?.vehicle?.brand} {review.reservation?.vehicle?.model}</h2><span className="text-warning" aria-label={`${review.rating} estrelas`}>{'★'.repeat(review.rating)}{'☆'.repeat(5-review.rating)}</span></div><p className="text-secondary mb-0">{review.comment}</p></article></Col>)}</Row>}
